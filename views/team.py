"""Team & tracks. Only OWNER_EMAILS (Aiyana) assigns track directors. A track's
director (and founders) build that track's team from the full user pool."""
...
def team():
    ...
    can_assign = core.can_assign_directors(role, user["email"])  # owner only
    ...
    if can_assign:
        with tab_objs[0]:
            ...
            dopts = {"— none —": None} | {
                f'{u["name"]} ({core.ROLE_LABELS[u["role"]]})': u["id"] for u in dir_people}
            if len(dopts) == 1:
                st.info("No directors yet. Add emails to DIRECTOR_EMAILS; meanwhile you "
                        "can manage every track's team in the next tab.")
